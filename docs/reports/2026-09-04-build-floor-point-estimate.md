...
| 2 | **Rebate cap** | `margin − m_min − floor` with the bound floor (measured path) — in effect 0 | `margin − m_min − floor_point`; the 8/16 `margin // 2` hand fraction is GONE from the measured path (already was); a floor-0 cell may rebate up to `margin − m_min`, a losing cell only what the margin holds BEYOND its measured loss (`margin − m_min − |shortfall|` — usually 0 on a 1–3¢ tier: 6 of 223 losing-cell quotes tonight kept any room; review fix S3); the fee floor (`m_min`, review fix M3), the ES-value cap and the exposure-backed rule are untouched | replay: rebate room on **13,077 / 20,000** post-8/20 quotes under the point rule vs **202** under the wire's upper rule (tonight: 779 / 1,158 vs 41 as of ~00:00Z; refreshed 00:41Z: **1,034 / 1,531 vs 56** — review fix S5) |
...
## Review fixes (fix pass 2026-09-05 00:30–01:05Z, same branch)

Adversarial review verdict SHIP_WITH_FIXES: one must-fix (M1) and five should-fix
(S1–S5). Bot LIVE on `main` throughout; the store was opened read-only only; every
heavy job (gate tiers, counterfactual, suite) ran at BelowNormal, one at a time.

| # | Review item | Verdict | What was done | Evidence |
|---|---|---|---|---|
| M1 | Rule 9 not run on the branch: run fast AND pre-ship from a `VITALS_DATA_DIR` snapshot (store copy, no live tape) before relighting | **DONE — fast 8/8 GREEN; pre-ship V6 RED = the adjudicated stale-ledger class** | NEW `tools/vitals/snapshot.py`: copies exactly the two tables the fast/pre-ship tiers read (`daily_ruin_anchors` 2 rows, `position_ledger` 3,970 rows — schema, rows, indexes; read-only URI, one bounded `SELECT *` each; 3.8 MB) into a directory that holds NO `live_*.log`, so `derive` takes the cached tape facts and reads bankroll/open book from the copy (refuses a directory with tape; prints what the gate will derive). Runs from the snapshot at BelowNormal: **fast 8/8 GREEN in 90 s** (V1 cap scope, V2 quoting liveness, V3 loop liveness under the 19-market fan-out, V4 observed-tier rate budget, V5 funnel conservation, V7 metric truth, V8 supervision coverage, V9 process hygiene over 256 boots × 3 shapes); **pre-ship V6 RED**: "the MC does NOT fit at the LIVE book (4,078 ms vs 2,996 ms)" on the snapshot's **338 open rows** (1× build 114 ms + MC 1,293 ms × 3; 3× 20.6 s; 5× 41.6 s) — **321 of the 338 opened BEFORE 2026-09-04** (88 on 8/09, 31 on 8/15, 30 on 8/27; 17 opened tonight): the same stale-ledger artifact the 18:45 relight gate adjudicated at 434 rows (5th ship-through) less the 28 phantom rows and the settlements since. This branch does not touch `compute_book_risk` / the candidate gate / `build_book_model` (review: AST-identical to `main` outside `risk/retained_edge_floor.py`), so V6 measures the ledger, not this change; the 6th ship-through decision is the orchestrator's. Tape facts for the run: `tools/vitals/tape_facts.json` as refreshed on `main` by the pre-relight scan (uncommitted there) — 271 CLOSED logs / 278.29 GB, tier observed 2026-08-27, 256 boots 3.2–59.6 s, fan-out 19, `live_n_positions_max` 139 — copied in for the run, then the branch's committed cache (the 8/20 scan) restored: committing main's dirty file here would collide at merge; the orchestrator owns it. Bankroll graded against: `daily_ruin_anchors[2026-07-16]` = $2,050.41 — the store's latest anchor, the same value the live gate reads (a stale anchor; for the vitals owner, not this build). `derive.py` untouched (`prove.py` therefore not re-run). | `tools/vitals/snapshot.py`; session scratchpad `gate_fast.out`, `gate_preship.out`, `vitals_snapshot/snapshot_provenance.json` |
| S1 | Half the wire's floor is a POOL point whose sign is not measured (soccer \|μ\|/SE 0.16 → 0 → 403 quotes with room); derived repair = the cell thinness test one level up | **MEASURED, read-only — the proposed derived repair is UNDETERMINED on today's grade; recorded as ratification + a follow-up joined to O3** | Second EB level over the sport pools with the same derivation as the cell level (τ²_global = max(0, Var_sports(μ_s) − mean_s(SE_s²)), w_s = τ²/(τ² + SE_s²), post_s = w_s·μ_s + (1 − w_s)·μ_global, thin = w_s < ½), global pool n 3,335 / G 1,666 / mean −57.3 cc/ct / SE 151.9: with all four pools **τ_global 1,598 cc** (between 2,817,013 vs within 263,327) → weights esports 0.80, mlb 0.99, other 0.94, soccer 0.92 → NO pool is thin, shrunk pool points {esports 0, mlb 84, other 0, soccer 0} vs today's {0, 85, 0, 0} — a 1 cc no-op. But the between-sport dispersion is carried by the 3-cluster "other" pool (+2,786 cc): **without it τ²_global = 0 → every pool thin** → each takes the global point (58 cc) or the largest pool point (85 cc), i.e. soccer/esports would floor at 0.58–0.85 ¢ instead of 0. A rule whose answer flips on one G = 3 pool is the O3 class (a hierarchical variance prior, now owed at BOTH levels), not a missing pool test — so it is NOT wired, and the operator ratification stands exactly as the builder named it: floor-0 pools rebate up to `margin − m_min`, bounded by the ES-value cap / exposure-backed rule / fee floor (tonight 572 quotes on 192 cells looser than the 8/16 `margin // 2`). The live log line already carries the read's inputs (`pool_mean_cc_by_sport`, `pool_se_cc_by_sport`, `tau_cc_by_sport`). | scratchpad `s1/s1_pool_level_eb.py`, `s1/s1_variant.py` (plain arithmetic over the live estimator's `PoolStats`) |
| S2 | 61/206 populated cells have \|post\| < 1 SE; the ML×ML cross-game floor (171 cc, \|post\|/SE 0.28) is a coin-flip on sign — do not call it a measured loss | **DONE** | `summarize()` logs `n_populated_sign_unresolved` (populated cells with \|post\| < posterior SE; live **61 = 31 losing + 30 at-fee**) — the watch list for the pre-registered ≥ 2-week read; module docstring, the losing-cells section above and the README row reworded (a point on a noisy record, not a measured loss; ML×ML at \|post\|/SE 0.29 by the live grade). New test `test_a_cell_whose_sign_is_unresolved_is_counted_not_trusted` (a −40/+38 alternating cell: populated, \|post\| < SE, still floors at its point, counted once); the existing summary pin asserts 0 on strongly-signed cells. Blast radius: a log-line field; `estimate_retained_floor` and the published table are byte-identical. | `src/combomaker/risk/retained_edge_floor.py`, `tests/test_retained_edge_floor.py` (16 tests) |
| S3 | The losing-cell rule is "no rebate BEYOND the measured loss" (cap = max(0, margin − m_min − \|post\|)), not "no rebate" | **DONE** | Reworded in the module docstring, `tools/proto_floor_point.py`, row 2 of the top table, the losing-cells section and the README row, with the measured exception on record: tonight 6 of 223 losing-cell quotes kept room (KS×KS cross, floor 12 cc, post −0.12 ¢ SE 11.4 ¢: 5/8 open, mean cap 26 cc; KS×4 cross, floor 9 cc: 1/1, 60 cc). | grep "no rebate" over the touched files |
| S4 | Test-honesty: one test DELETED, two renamed+changed, one inverted, three added — "no deletions" is not literally true | **DONE — stated literally in §Tests** | Deleted `test_tail_quantile_is_z_for_many_clusters_and_fails_closed_for_few` (its subject `tail_quantile` was removed with the rule); renamed + re-pinned 2; inverted + renamed 1 (the 3-row pool floors at 0); added 4 in the build + 1 here (`main` 12 → 16: 4 names gone, 8 new — the review's "three added" undercounted by one). Every changed pin carries the measured reason and an inequality against build A's value. | §Tests |
| S5 | Refresh tonight's-range numbers from the current store (1,158 → 1,525+) | **DONE at 00:41Z** | 1,531 `quote_sent` from rowid 134174705: rebate room **1,034 / 1,531 (67.5%)** under point vs **56** under the wire's upper vs 1,278 fee-only; NO tier to zero; parity 1,531/1,531; no-rebate 30 of 286 cells (250 quotes). Same proportions as the 00:00Z table (67%). Relight-watch expectations in NEXT STEPS updated. | §Counterfactual (refreshed paragraph); scratchpad `cf_tonight.out` |

Fix-pass gates: ruff clean on the 4 touched files; mypy clean on
`risk/retained_edge_floor.py` and `tools/proto_floor_point.py` (`tools/vitals/snapshot.py`
type-checks clean itself; the one mypy error it surfaces is `derive.py:343`'s
pre-existing `model_fields` attr on `main`); touched tests + full suite: see §Gates.
...
**Refreshed at 00:41Z (review fix S5)** — the same range now holds **1,531**
`quote_sent` (the 18:45, 19:29 and 20:10 ET boots): rebate room **1,034 / 1,531
(67.5%)** under the point rule vs **56** under the wire's upper vs 1,278 fee-only;
open point/upper/fee-only per tier: soccer:razor60 170/44/176, soccer:mains100
243/11/268, mlb:mains100 41/0/197, mlb:ladder300 181/0/192, soccer:ladder200
154/0/157, mlb:razor60 34/0/45, mlb:ladder200 84/0/109, soccer:ladder300 82/0/82,
mlb:ladder250 34/0/41, mixed+esports 11/1/11; NO tier to zero; parity prototype ==
live through `construct_quote` **1,531/1,531**; cap loosened beyond `margin // 2`
on 572 quotes / 192 cells; no-rebate cells 30 of 286 quoted (250 quotes), led by
ML×ML cross-game 99. Same proportions as the 00:00Z table.
...
0.75 ¢ and the mains' 1 ¢: no rebate on that shape while the point sits there.
**Its sign is NOT resolved** (review fix S2): |post|/SE = 0.29, a coin-flip — 61 of
the 206 populated cells (31 losing, 30 at-fee) have |post| < 1 posterior SE, so
171 cc is a point on a noisy record, not a measured loss; the next sweep can move
it either way (which would flip the whole razor/mains class to full room), which
is exactly why the ≥ 2-week trajectory read is pre-registered. `summarize` now
logs the count (`n_populated_sign_unresolved`). And "no rebate" on a losing cell
means no rebate BEYOND the measured loss (cap = `margin − m_min − |shortfall|`,
review fix S3): tonight 6 of 223 losing-cell quotes kept room (KS×KS cross,
floor 12 cc: 5/8 open, mean cap 26 cc; KS×4 cross, floor 9 cc: 1/1, 60 cc).
...
non-zero pin. **Disclosure (review fix S4), literally:** one test was DELETED —
`test_tail_quantile_is_z_for_many_clusters_and_fails_closed_for_few` — because
its subject `tail_quantile` (build A's Student-t quantile, the muting
mechanism) was removed from the module; two tests were renamed and re-pinned
(`test_floor_is_the_z_upper_bound_of_adverse_selection` →
`test_floor_is_the_shrunk_point_shortfall`,
`test_thin_cell_takes_the_sport_pools_upper_bound` → `…_point`); one pin was
INVERTED and renamed (the 3-row +27.9 ¢ pool floored > 0 under build A —
`test_a_three_row_pool_cannot_publish_a_zero_floor_off_an_outperformance` — and
floors at 0 by its point: `test_an_outperforming_pool_floors_at_zero_by_its_point`);
FOUR tests were added in the build (`test_point_floor_is_the_measured_loss_and_never_negative`,
`test_unknown_sport_takes_the_largest_pool_point`, the two properties) and one in
the fix pass — `main` 12 → 16 (`git diff main` on the test names: 4 gone, 8 new). "No deletions" would
be false; "no weakening" holds in this sense: every removed assertion's subject
was removed with the rule, and every replacement is an equality on the new rule
plus an explicit inequality against what the old rule would have published.
...
| full suite (BelowNormal, worktree) | build: **4,044 passed / 0 failed / 3 deselected** in 366 s (main: 4,041; +3 = the new pins net of the removed quantile tests); **fix pass: 4,045 passed / 0 failed / 3 deselected in 317 s** (+1 = the sign-unresolved test); touched files `test_retained_edge_floor.py` + `test_quote_fee_floor.py`: 124 passed in 6.0 s |
| vitals fast + pre-ship (rule 9) — FIX PASS | **RUN from a store SNAPSHOT (review fix M1; new `tools/vitals/snapshot.py`, see §Review fixes): fast 8/8 GREEN (90 s); pre-ship V6 RED = the adjudicated stale-ledger class (338 open rows, 321 opened before 9/4; this branch does not touch that path) — the 6th ship-through decision is the orchestrator's.** Build-time note kept for the record: the run I started at 23:52Z (`VITALS_DATA_DIR` = the live data dir, BelowNormal) spent 1,400 CPU-s in `scan_tape` — the cache manifest includes the CURRENT live log, which grows every second, so on a live machine the fast tier rescans the ENTIRE tape: 274 files / **266 GB** (the docstring says 10 GB); it had read 180 GB by 00:14Z from the same drive the live store writes; the 19:29 ET boot was stall-killed at 00:09:07Z (`supervisor_loop_wedged` maintenance age 61.2 s > 60.5 s — the known saturated-store wall; checkpoint failures and `fill_ledger_write_stalled` predate my scan by 20 min, but the scan's IO cannot be excluded as a contributor) → I stopped the scan at 00:15Z. Process priority does not lower disk IO priority. Run it from a snapshot data dir, or after the manifest excludes the live log (O5) |
...
- Orchestrator: vitals fast 8/8 GREEN + pre-ship V6 RED are on record from the
  snapshot (§Review fixes) — adjudicate V6 (the stale-ledger class, 6th
  ship-through; nothing on that path changes here), merge
  `build/floor-point-estimate`, then the relight window: the first maintenance
  tick must log `retained_floor_estimate` with `rule=shrunk_point`,
  `pool_floor_cc` ≈ {mlb 85, soccer 0, esports 0, other 0},
  `n_populated_losing`/`n_populated_at_fee` ≈ 88/118 and
  `n_populated_sign_unresolved` ≈ 61; sends/min stays 300–460; then the rebate
  should be visible again in `quote_sent` skew on the ladders (the 8/16 read:
  retained edge after rebate by tier, pooled ≥ 2 weeks). Post-merge gate runs
  on the live box: `python -m tools.vitals.snapshot --store <live store> --out
  <dir>` then `VITALS_DATA_DIR=<dir> python -m tools.vitals.gate [--tier
  pre-ship]` — never the live data dir while the bot writes. Also owed on
  `main`: commit the refreshed `tools/vitals/tape_facts.json` (the pre-relight
  scan, 271 closed logs) that is sitting uncommitted there.
- S1 follow-up (derived, with O3): the pool-level thinness test is undetermined
  on today's grade — its answer flips on the 3-cluster "other" pool (§Review
  fixes) — so a hierarchical variance prior is owed at BOTH levels before any
  pool can be called thin; until then the ratification stands (floor-0 pools
  rebate up to `margin − m_min`, bounded by the ES-value cap / exposure-backed
  rule / fee floor).
