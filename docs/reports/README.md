...
| 2026-07-09 | **MLB/baseball SGP finalization** | **IN PROGRESS** | operator directive: soccer is strong/nearly done → baseball is the active workstream. Multi-agent pass launched (classify ALL MLB prop families strictly from Kalshi docs+API+tape; re-verify measured ρ on Retrosheet incl. extension to 2005-25 + independent re-derivation; measure every pair Kalshi allows in combos; adversarial verify; unknowns queue). Gate to finalize: numbers verified + backed by deep data + tape backtest + ZERO unknown legs. Dated results report to follow when the pass completes |
