...
class DnpScalarConfig(StrictModel):
    """DNP scalar-settlement guard for single-player-driven combos
    (pricing/dnp_scalar.py, 2026-08-06 build; forensics
    docs/reports/2026-08-06-dnp-scalar-pickoff.md — 3 Marte combos sniped
    −$39.75 by a taker pricing the scratch at h=1).

    SHADOW default OFF: with ``enabled=False`` the pricer is BYTE-IDENTICAL
    to the pre-guard pricer (era-replay proven). Armed, two mechanisms fire
    for combos whose legs ALL ride one player (UNKNOWN entity fails closed
    into scope): the void-branch fair mixture (h = MEASURED per-family DNP
    hazard, baseline table in pricing/dnp_scalar.py regenerated by
    tools/measure_dnp_hazard.py + live settled-leg merge — never typed) and
    the sniper ask floor at the DNP settlement value ⌊∏s⌋ ("we either profit
    or don't lose" on a scratch). No other combo shape is touched."""
...
class PricingConfig(StrictModel):
    ...
    # DNP scalar guard (2026-08-06). SHADOW default off — see DnpScalarConfig.
    dnp_scalar: DnpScalarConfig = Field(default_factory=DnpScalarConfig)
