...
class CorrelationConfig(StrictModel):
    ...
    pair_rho_by_sport: dict[str, dict[str, float]] = {
        ...
        "mlb": {
            ...
            # flow). With these tabled, NO same-game MLB pair except
            # moneyline|spread (deliberately untabled — TODO(route) below)
            ...
            # -- [C-sibling] spread × props, NEUTRALIZED (RESOLVER-PHASE +
            # MEASURE) -- spread is team-SIGNED like the moneyline ("wins by
            # more than N-0.5"), so every spread×prop correlation sign-FLIPS
            # with the prop player's team vs the spread team — the same axis
            # as the [C] block above, and the same remedy: no unoriented
            # scalar is right, ship 0.00 with a sign-spanning band until the
            # ML-orientation resolver ships (RESOLVER-PHASE) and the oriented
            # values are measured from the parsed Retrosheet npz (MEASURE —
            # DO-8). Expected magnitudes track the ml|prop siblings
            # (±0.23-0.37) modulated by the margin threshold.
            ...
            "rfi|spread": 0.00,
            # rfi × batter props: mildly POSITIVE labeled priors (+0.05..+0.10
            # per the sweep sketch) — an early run and a batter's counting
            # stats share the game's scoring environment. Points seeded by
            # one-factor coherence through the MEASURED total column
            # (rho(rfi,X) ≈ rfi|total 0.37 × X|total), clamped to the
            # sketch's +0.10 ceiling: hit 0.37x0.25≈0.09, hr 0.37x0.24≈0.09,
            # tb 0.37x0.27≈0.10, hrr 0.37x0.40≈0.15 → capped at 0.10 (hrr
            # carries the widest band for the cap). MEASURE-BEFORE-TIGHTEN
            # (DO-8): measure from the existing parsed npz before narrowing
            # any of these bands.
            "player_hit|rfi": 0.09,
            "player_hr|rfi": 0.09,
            "player_tb|rfi": 0.10,
    ...
    pair_rho_uncertainty: dict[str, float] = {
        ...
        # spread × props: sign-spanning until resolver + measurement (DO-8)
        "mlb:player_ks|spread": 0.20,
        "mlb:player_hit|spread": 0.20,
        "mlb:player_hr|spread": 0.20,
        "mlb:player_hrr|spread": 0.20,
        "mlb:player_tb|spread": 0.20,
        "mlb:rfi|spread": 0.15,             # ⊥ prior (mirrors measured ml|rfi 0.00)
        # rfi × props: labeled priors — MEASURE-BEFORE-TIGHTEN (DO-8)
        "mlb:player_hit|rfi": 0.15,
        "mlb:player_hr|rfi": 0.15,
        "mlb:player_tb|rfi": 0.15,
        "mlb:player_hrr|rfi": 0.20,         # one-factor implied 0.15 > the capped +0.10 point
    }
