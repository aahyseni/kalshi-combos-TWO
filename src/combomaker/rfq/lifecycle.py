...
class QuoteLifecycle:
    ...
    def _kill_anchor_readout(self, snap: object) -> dict[str, object]:
        ...
        No ``det_max_backstop_cc``: the det-max demotion was WITHDRAWN
        2026-07-31 as a measured risk regression, so there is no second wall to
        publish and reporting one would imply a ceiling that does not exist.
        ...
        try:
            ...
            out: dict[str, object] = {"kill_line_cc": kill_line_cc}
    ...
    def _build_candidate_gate_inputs(
        ...
        *,
        ...
        return CandidateBookRiskInputs(
            ...
            # KILL-ANCHORED BOOK GATE (2026-07-29): read off the SAME
            # ``RiskLimits`` the quote-time cap enforces (not a second config
            # copy), so cap and gate can never disagree about the anchor. The
            # KILL line rides along as a float. det-max is NOT re-anchored on
            # either site — the demotion to a ruin-derived 0.70 backstop was
            # WITHDRAWN 2026-07-31 as a measured risk regression (see
            # ``risk/limits.RiskLimits.kill_anchored_book_gate``).
