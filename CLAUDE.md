...
| 9 | **Correlation-adaptive risk caps (operator spec 2026-07-22).** Risk caps are NOT static config — they DERIVE nightly (`risk/cap_family.py`) from MEASURED within-game vol σ₁ and cross-game correlation → G_eff (`risk/pnl_measurement.py`), around ONE operator input: the **drawdown tolerance `kill_anchor`** (`adaptive_caps_kill_anchor`, default 0.12). `f_slate` is SOLVED so the KILL sits at `k_trip`=5σ of a night's P&L; `game=f_slate/expected_games` (expected_games auto-counted from the live slate); `daily`/`drawdown` = `k·sigma_day` from the deployed f_slate; book caps = 1.3×MC. A **fast-tighten / slow-loosen ratchet** (cross-ρ ≥ 0.05 blocks any cap increase; the formula shrinks G_eff as cross-ρ rises) reacts within the slate; loosening needs a stable multi-night read. A **validation guard** rejects any (f_slate, kill_anchor) pair where the KILL fails to cover the 4σ drawdown (the old-model self-destruct), and startup logs projected P(KILL over 60 nights). Clamped PROVISIONAL at f_slate ≤ 0.15 until MLB σ₁/ρ are measured on real nights. `derived_cap_engine.py` swaps the derived `RiskLimits` onto the live `LimitChecker`; enforcement/halt mechanics unchanged. See `docs/reports/2026-07-22-cap-family-spec-reconciliation.md` | operator spec; caps calibrated for 1–2-game WC nights self-destruct on 10–12-game MLB slates (KILL fires 99.9% of campaigns) — the KILL anchor, not conservatism, was the binding constraint |
