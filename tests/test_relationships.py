...
# --- Family 4: single-match ADVANCE ⟺ regulation MONEYLINE (KXWC knockout only) --
# KXWCGAME = regulation (90') result; KXWCADVANCE = wins the match (reg/ET/pens).
# Single-match: reg-win(T) ⟹ advance(T); advance(T) ⟹ ¬advance(opponent).
# SOURCE OF TRUTH (prod RFQ tape 2026-07-08): KXWCADVANCE is a head-to-head
# per-game market (two team outcomes), knockout-window only; KXWCGAME is 3-way
# incl. TIE. Reachable combo orientations: moneyline only on YES (NO blocked);
# advance on YES or NO. Observed advance×game combos all carry ml on TIE.
ML_ENG = "KXWCGAME-26JUL05MEXENG-ENG"
ADV_ENG = "KXWCADVANCE-26JUL05MEXENG-ENG"


def test_adv_same_team_ml_yes_adv_yes_is_containment_subset_ml() -> None:
    """SAME team {ml-yes, adv-yes}: reg-win ⊆ advance, so joint = P(reg-win);
    subset points at the moneyline leg (index 0)."""
    legs = (leg(ML_MEX, ML_EV, "yes"), leg(ADV_MEX, ADV_EV, "yes"))
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.CONTAINMENT
    assert rel.containment == (0, 1)


def test_adv_same_team_ml_yes_adv_no_is_impossible_not_farmable() -> None:
    """SAME team {ml-yes, adv-no}: a regulation win but NOT advancing is impossible
    in KNOCKOUT — but that's metadata-dependent (a group team can win yet not
    advance), so farm rule #2 forbids farming it. IMPOSSIBLE, NON-farmable (v1
    no-quote; enable only behind a knockout guard)."""
    legs = (leg(ML_MEX, ML_EV, "yes"), leg(ADV_MEX, ADV_EV, "no"))
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.IMPOSSIBLE
    assert rel.farmable is False


def test_adv_same_team_ml_yes_adv_no_is_impossible_at_any_size() -> None:
    """The win-but-not-advance contradiction makes the WHOLE combo unpayable, so it
    fires (IMPOSSIBLE) even buried in a >2-leg combo — but non-farmable (see above)."""
    legs = (
        leg(ML_MEX, ML_EV, "yes"),
        leg(ADV_MEX, ADV_EV, "no"),
        leg("KXWCTOTAL-26JUL05MEXENG-3", "KXWCTOTAL-26JUL05MEXENG", "yes"),
    )
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.IMPOSSIBLE
...
def test_adv_opp_team_adv_yes_ml_yes_is_impossible_not_farmable() -> None:
    """OPP team {adv-A-yes, ml-B-yes}: reg-win(B) ⟹ B advances ⟹ A does not, so
    both can't settle YES → IMPOSSIBLE. NOT farmable (opponent + exactly-one-
    advances inference, not a same-leg tautology) — v1 no-quote."""
    legs = (leg(ADV_MEX, ADV_EV, "yes"), leg(ML_ENG, ML_EV, "yes"))
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.IMPOSSIBLE
    assert rel.farmable is False


def test_adv_opp_team_adv_no_ml_yes_is_containment_subset_ml() -> None:
    """OPP team {adv-A-no, ml-B-yes}: reg-win(B) ⊆ ¬advance(A), so joint =
    P(reg-win-B); subset points at the moneyline-B leg (index 1)."""
    legs = (leg(ADV_MEX, ADV_EV, "no"), leg(ML_ENG, ML_EV, "yes"))
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.CONTAINMENT
    assert rel.containment == (1, 0)


def test_adv_ml_tie_is_possible_not_classified() -> None:
    """The OBSERVED tape cell: advance × moneyline-TIE. A regulation draw does not
    pin who advances → the advance family must NOT fire; falls to the copula."""
    legs = (leg(ADV_MEX, ADV_EV, "yes"), leg(ML_TIE, ML_EV, "yes"))
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.OK
    assert rel.same_event_groups == ((0, 1),)


def test_adv_ml_no_side_is_not_classified() -> None:
    """Moneyline-NO is unreachable in combos and is NOT wired: {ml-no, adv-no}
    never pins → falls to the copula, never a containment/impossible."""
    legs = (leg(ML_MEX, ML_EV, "no"), leg(ADV_MEX, ADV_EV, "no"))
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.OK


def test_adv_same_team_containment_buried_is_unknown() -> None:
    """A bare-pair containment buried in a >2-leg combo → UNKNOWN (widen-or-no-
    quote), never a copula guess."""
    legs = (
        leg(ML_MEX, ML_EV, "yes"),
        leg(ADV_MEX, ADV_EV, "yes"),
        leg("KXWCTOTAL-26JUL05MEXENG-3", "KXWCTOTAL-26JUL05MEXENG", "yes"),
    )
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.UNKNOWN
...
def test_adv_different_game_is_not_classified() -> None:
    """Advance and moneyline of DIFFERENT games carry no logical relation."""
    legs = (
        leg(ADV_MEX, ADV_EV, "yes"),
        leg("KXWCGAME-26JUL06ARGEGY-ARG", "KXWCGAME-26JUL06ARGEGY", "no"),
    )
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.OK


# --- SINGLE-MATCH SCOPING: two-legged UCL/UEL/UECL must NEVER be classified ------
# A regulation win in ONE leg of a two-legged tie does NOT imply advancing the
# aggregate, so the containment/impossible/farm logic must never touch a KXUCL*
# advance/moneyline/spread leg. Gated by exact KXWC* series prefix.
UCL_ADV_RMA = "KXUCLADVANCE-26MAY01RMABAR-RMA"
UCL_ADV_EV = "KXUCLADVANCE-26MAY01RMABAR"
UCL_ML_RMA = "KXUCLGAME-26MAY01RMABAR-RMA"
UCL_ML_BAR = "KXUCLGAME-26MAY01RMABAR-BAR"
UCL_ML_EV = "KXUCLGAME-26MAY01RMABAR"
UCL_SP_RMA2 = "KXUCLSPREAD-26MAY01RMABAR-RMA2"
UCL_SP_EV = "KXUCLSPREAD-26MAY01RMABAR"


def test_ucl_advance_moneyline_same_team_is_not_impossible() -> None:
    """CRITICAL single-match guard: a two-legged UCL {ml-yes, adv-no} must NOT be
    classified IMPOSSIBLE/farmable (a reg win in one leg ⇏ advancing the tie).
    It falls through to the ordinary same-game copula group."""
    legs = (leg(UCL_ML_RMA, UCL_ML_EV, "yes"), leg(UCL_ADV_RMA, UCL_ADV_EV, "no"))
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.OK
    assert rel.farmable is False
    assert rel.containment is None
...
def test_ucl_advance_moneyline_same_team_yes_is_not_containment() -> None:
    """The UCL two-legged {ml-yes, adv-yes} must NOT be pinned as containment."""
    legs = (leg(UCL_ML_RMA, UCL_ML_EV, "yes"), leg(UCL_ADV_RMA, UCL_ADV_EV, "yes"))
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.OK
    assert rel.containment is None


def test_ucl_opp_team_advance_is_not_impossible() -> None:
    """UCL opp-team {adv-A-yes, ml-B-yes} must NOT be IMPOSSIBLE (two-legged: a
    reg win by B in one leg does not eliminate A from the aggregate tie)."""
    legs = (leg(UCL_ADV_RMA, UCL_ADV_EV, "yes"), leg(UCL_ML_BAR, UCL_ML_EV, "yes"))
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.OK


def test_ucl_spread_moneyline_is_not_classified() -> None:
    """UCL spread|moneyline is also gated off (single-match KXWC scoping)."""
    legs = (leg(UCL_SP_RMA2, UCL_SP_EV, "yes"), leg(UCL_ML_BAR, UCL_ML_EV, "yes"))
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.OK


# --- Family 5: single-match regulation SPREAD ⟺ regulation MONEYLINE (KXWC) ------
# KXWCSPREAD-…-<TEAM>N = wins in regulation by margin ≥ N (N≥1) ⟹ that team wins.
# SOURCE OF TRUTH (prod RFQ tape 2026-07-08): reachable spread lines 2/3/4 both
# sides; the OBSERVED game×spread combos are all same-team {ml-yes, spread-no}.
SP_MEX2 = "KXWCSPREAD-26JUL05MEXENG-MEX2"
SP_MEX0 = "KXWCSPREAD-26JUL05MEXENG-MEX0"   # line 0 (never implies a win)
SP_EV = "KXWCSPREAD-26JUL05MEXENG"


def test_spread_same_team_spread_yes_ml_yes_is_containment_subset_spread() -> None:
    """SAME team {spread-yes, ml-yes}: spread ⊆ reg-win, so joint = P(spread);
    subset points at the spread leg (index 0)."""
    legs = (leg(SP_MEX2, SP_EV, "yes"), leg(ML_MEX, ML_EV, "yes"))
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.CONTAINMENT
    assert rel.containment == (0, 1)


def test_spread_same_team_spread_no_ml_yes_is_possible() -> None:
    """The OBSERVED cell: {spread-no, ml-yes} (won by < N) — possible, falls to
    the copula (grouped)."""
    legs = (leg(SP_MEX2, SP_EV, "no"), leg(ML_MEX, ML_EV, "yes"))
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.OK
    assert rel.same_event_groups == ((0, 1),)


def test_spread_opp_team_spread_yes_ml_yes_is_impossible_not_farmable() -> None:
    """OPP team {spread-A-yes, ml-B-yes}: A wins reg by ≥N AND B wins reg — both
    can't win the same match → IMPOSSIBLE, NOT farmable (cross-outcome pin)."""
    legs = (leg(SP_MEX2, SP_EV, "yes"), leg(ML_ENG, ML_EV, "yes"))
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.IMPOSSIBLE
    assert rel.farmable is False


def test_spread_opp_team_spread_no_ml_yes_is_possible() -> None:
    """{spread-A-no, ml-B-yes} does not pin the result → possible → copula."""
    legs = (leg(SP_MEX2, SP_EV, "no"), leg(ML_ENG, ML_EV, "yes"))
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.OK


def test_spread_tie_spread_yes_ml_tie_yes_is_impossible_not_farmable() -> None:
    """TIE {spread-S-yes, ml-TIE-yes}: a win by ≥N is not a draw → IMPOSSIBLE,
    NOT farmable (cross-outcome result pin)."""
    legs = (leg(SP_MEX2, SP_EV, "yes"), leg(ML_TIE, ML_EV, "yes"))
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.IMPOSSIBLE
    assert rel.farmable is False


def test_spread_tie_spread_no_ml_tie_yes_is_possible() -> None:
    """{spread-S-no, ml-TIE-yes}: no team won by ≥N and it's a draw — possible."""
    legs = (leg(SP_MEX2, SP_EV, "no"), leg(ML_TIE, ML_EV, "yes"))
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.OK


def test_spread_line_below_one_is_not_classified() -> None:
    """A spread line < 1 (e.g. a 0/handicap suffix) need not imply a win, so it is
    NOT classified — falls to the copula (fail-closed parse)."""
    legs = (leg(SP_MEX0, SP_EV, "yes"), leg(ML_ENG, ML_EV, "yes"))
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.OK


def test_spread_same_team_containment_buried_is_unknown() -> None:
    """Spread|moneyline containment buried in a >2-leg combo → UNKNOWN."""
    legs = (
        leg(SP_MEX2, SP_EV, "yes"),
        leg(ML_MEX, ML_EV, "yes"),
        leg("KXWCTOTAL-26JUL05MEXENG-3", "KXWCTOTAL-26JUL05MEXENG", "yes"),
    )
    rel = classify_legs(legs, ExplodingProvider())
...
def test_spread_opp_team_impossible_fires_buried() -> None:
    """The opp-team spread contradiction unpayable-ifies the whole combo, so it
    (non-farmable) fires even buried in a >2-leg combo."""
    legs = (
        leg(SP_MEX2, SP_EV, "yes"),
        leg(ML_ENG, ML_EV, "yes"),
        leg("KXWCTOTAL-26JUL05MEXENG-3", "KXWCTOTAL-26JUL05MEXENG", "yes"),
    )
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.IMPOSSIBLE
...
def test_spread_different_game_is_not_classified() -> None:
    """Spread and moneyline of DIFFERENT games carry no logical relation."""
    legs = (
        leg(SP_MEX2, SP_EV, "yes"),
        leg("KXWCGAME-26JUL06ARGEGY-EGY", "KXWCGAME-26JUL06ARGEGY", "yes"),
    )
    rel = classify_legs(legs, ExplodingProvider())
    assert rel.kind is RelationshipKind.OK
