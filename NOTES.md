...
| `getting_started/fixed_point_migration.md`: "`*_dollars` fields are fixed-point dollar strings with **up to 4 decimal places** (e.g. `"0.1200"`)" | **WRONG for the positions payload (2026-07-26).** `api-reference/portfolio/get-positions.md` says `FixedPointDollars` carries "**up to 6 decimal places** of precision", and the live wire uses all six. Read-only prod probe, 46 open rows: **46/46** `market_exposure_dollars` have SIX decimals and **21/46** are NOT a whole number of centi-cents (`"2.688790"` = 26.8879 cc). The MarketPosition schema has **no** int-cents `market_exposure` / `position` fields at all — `market_exposure` was absent from **46/46** rows, so the documented int-cents fallback is dead code. Fields present on all 46: `ticker`, `position_fp`, `market_exposure_dollars`, `total_traded_dollars`, `realized_pnl_dollars`, `fees_paid_dollars`, `last_updated_ts` | 4-decimal assumptions on `*_dollars` are UNSAFE. An exchange-reported **exposure** now parses via `core/money.exposure_cc_from_dollars_str` (ROUND_CEILING — a fail-closed at-risk figure is never booked below truth); the strict `cc_from_dollars_str` stays for prices/balances/revenue/settlement values. Wire pinned in `tests/fixtures/live_positions_20260726.json`; regression: `tests/test_exchange_exposure_precision.py` |
...
## Maker-fee section — assumption audit (2026-07-16, defense #6)

| row | assumption | where | provenance |
|---|---|---|---|
| MF1 | When Kalshi adds combo series to its maker-fee list, maker fills on those series pay the SAME quadratic schedule with the maker coefficient 0.0175 (fee = ceil(0.0175·C·P·(1−P)), ceiled to a centi-cent) — i.e. `FeeType.QUADRATIC_WITH_MAKER_FEES` in the existing `pricing/fees.py` model, no new formula | `ops/config.py FeeConfig.maker_fee_active_prefixes`, `rfq/lifecycle.py _effective_fee_type` | doc:official fee-schedule PDF (2026-06-29, maker 0.0175 verified) — **UNVERIFIED for the future combo maker-fee announcement** (rate/structure could differ); watch `GET /series/fee_changes`, and defense #3 reconciles predicted vs actual to the cent on the first real fill of an activated series |
...
| SR5 | The marginal-jump breaker exempts a leg from its jump/readability watch ONLY when the EXCHANGE confirmed the market non-live (a graded fact is cached, or the last successful GET /markets fetch reported status closed/determined/disputed/amended/finalized); for such a ticker the book leaving the feed is the normal permanent close transition, never the dead-feed signature. No exchange confirmation (never fetched / fetch failing / last seen live) ⇒ full watch retained (a genuinely dead feed on a live market still halts). Quote path unaffected (ungraded closed legs still price UNKNOWN) | `risk/breakers.py BreakerInputs.settled_tickers` + `_evaluate_marginal_jumps` skip/purge, `rfq/lifecycle.py settled_watch_exempt`, `marketdata/settled.py market_no_longer_live` | doc:openapi.yaml status enum (one-way close lifecycle, index-scan §10) + live halt evidence 2026-07-18 02:17Z (`live_20260718_settled_relight.log`); regression-pinned both directions in `tests/test_settled_marginals.py` |

## Account-standing + transfer-watch + receivables — assumption audit (2026-07-21, defense #6)

Doc verification (hard rule 4): fetched `docs.kalshi.com/api-reference/portfolio/
get-deposits.md`, `get-withdrawals.md`, `get-positions.md` 2026-07-21. Deposits AND
withdrawals share the status enum `pending|applied|failed|returned` with `applied` =
funds moved ("reflected in balance" / "deducted from balance"); `amount_cents`/
`fee_cents` int cents; `finalized_ts` unix ms nullable, stamps the terminal
transition. Positions: `market_exposure_dollars` = "Cost of the aggregate market
position in dollars"; query params `ticker`/`count_filter`/`limit≤1000`/`cursor`.

| row | assumption | where | provenance |
|---|---|---|---|
| AS1 | The settlement row appearing in GET /portfolio/settlements and the balance credit are the same exchange-side booking, so a balance poll whose request starts after the reconciler read the row contains the credited cash (the receivable drop rule) | `risk/balance.py _drop_confirmed_receivables` | **UNVERIFIED** — asserted from ledger-consistency reasoning; the 2026-07-19 incident proved `portfolio_value` and `balance` are NOT atomic across endpoints, so verify empirically on the next live settlement (compare settlements-row visibility vs the next balance poll) and record here |
...
## Bundesliga + Saudi Pro League wire — assumption audit (2026-08-26, defense #6)

| # | Assumption | Where | Source |
|---|-----------|-------|--------|
| BS1 | Kalshi's only Saudi soccer base ticker is `KXSAUDIPL` (Saudi Pro League); SAUDIPRO/SPL/ROSHN/SAUDI/SAUDIPREM 404. Operator's "Saudi Premier League" = this league. | `legtypes._SPORT_KEYWORDS` (`SAUDIPL`) | live series API, recon fleet 2026-08-26 |
| BS2 | Both leagues settle 90'+stoppage regulation-only (no ET/pens), BTTS own-goals clause, 48h cancel/reschedule fair-price SCALAR clause ⇒ `farmable=False` inherits (series-scoped `_farm_certain` [KXWC] unchanged). | pricing/relationships (no change needed) | verbatim rules_primary/secondary scrapes, `docs/calibration/club_soccer_rules_pin.md` §2026-08-26 |
| BS3 | Expiration anchors: GAME = kickoff+3h, TOTAL/SPREAD/BTTS = kickoff+4h (same-fixture deltas measured both leagues) → yaml offsets 3.0167/4.0167, 48h max-pregame. Re-verify at first settlements per league. | `config/prod-live-wc.local.yaml` | measured live market objects 2026-08-26 |
| BS4 | `KXBUNDESLIGA2*` = Bundesliga 2 (DIFFERENT league, shared character prefix): classifies soccer (true facts) but is NOT admitted — the exact-family-with-dash allowlist style (`KXBUNDESLIGAGAME-` ⊄ `KXBUNDESLIGA2GAME-`) is the gate. `KXBBLGAME` = Bundesliga BASKETBALL, stays UNKNOWN (no string collision). | allowlist style + `test_club_soccer_wiring.py` pins | live series enumeration 2026-08-26 |
| BS5 | Saudi combo-collection membership: reverse probe by `associated_event_ticker` returns [] for ALL Saudi events, yet a live open RFQ carries a KXSAUDIPLGAME leg inside `KXMVECROSSCATEGORY-SHARD1-R` — membership TAPE-PROVEN; association appears to lag ~1 week pre-fixture (Bundesliga Sep 6 event also [], Aug 30 event returns all three collections). Pipeline keys on leg series, so no behavior depends on the probe. **UNVERIFIED-why-probe-lags.** | recon 2026-08-26 | exchange open-RFQ tape (RFQ 2bb7291c) |
| BS6 | Home/away frame for both leagues: ticker-first-token = home is CONSISTENT with all 8 markets read (incl. Frankfurt home 9/6) but **UNPINNED** — L1 protocol (schedule cross-check + one settled market per league) before DC home-advantage ever arms. DC currently prices from live legs without a home prior, so no behavior depends on it. | `club_soccer_rules_pin.md` | recon 2026-08-26 |
| BS7 | `balance_breakdown` now enumerates FOUR shards (2 & 3 empty, appeared while the bot was down); the whole-book merge is dynamic (any shard count) — verified at `ops/quote_app.py:1035-1046`. `tools/mvec_eligibility_scan.py` EXPECTED_COLLECTIONS still assumes 2 collections and would exit 1 (known, 8/12 flag, still open). | `quote_app.WholeBookBalanceSource` | live balance read 2026-08-26 |
