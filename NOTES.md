...
| L6 | **Partial-failure fail-closed.** `pregame_mid` returned `None` for BOTH a legit empty window AND an API error (non-429 got zero retries), and `best_mid` conflated them ⇒ an error on the true-main rung crowned a runner-up line, written + marked done forever. **Fix:** `pregame_mid` retries transient 429/5xx with backoff and raises `_ProbeError` (distinct from `None`) on persistent failure; `fetch_main`/`fetch_spreads` skip the WHOLE game on any `_ProbeError` (un-done → retried next run) rather than record a wrong main line (quiet-failure defense #2) | `tools/fetch_kalshi_history.py` | fail-closed on error |
...
Clean-data re-validation (WNBA/MLB/NBA re-fetched with the hardened full-ladder fetcher) is the remaining "verify afterwards" step; K9 numbers appended when the re-fetch completes. 724 tests green, mypy strict, ruff clean.

**OPEN operator decisions (from the re-verify, non-blocking):**
1. **Gate verdict policy** (`open-gate-significance`, CONFIRMED gate-policy wart): the native validators AND all three metrics including team-win×over, which K8/K5/K4 establish has ~zero true dependence in these sports — so a noise loss on it flips the machine verdict to "does NOT beat"/exit 1 even when the decisive cover/triple metrics win by ~0.4 nats. Conservative (false-fail only keeps a model disabled, capital-safe), but the verdict TEXT is misleading. Recommended: demote win×over to diagnostic (never vetoes) + report ties as ties. Owner: operator.
2. **WNBA enablement** now rests almost entirely on the operator request + spread/team-total coherence: the frame correction shrank the WNBA ml×total edge to ~noise (1.25197 vs 1.25283, inside the tie band). Keep WNBA enabled or gate to ml-only until a WNBA shadow-settlement gate exists? Owner: operator. Also: WNBA blob home/away order (away=prefix) was hand-verified for NBA/MLB, only inferred for WNBA — the (tiny) ρ sign depends on it; a live `KXWNBAGAME` "at"-title check would pin it.
